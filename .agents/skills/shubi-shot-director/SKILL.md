...
- Run `pose inspect --json` before composition inspection. A diagnostic `fail` blocks save/load and export; a `check` remains explicit and never becomes an automatic visual pass.
