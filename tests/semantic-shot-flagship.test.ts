...
describe("v1 semantic shot flagship black box", () => {
  it("keeps the public flagship submission executable", () => {
    const input = JSON.parse(
      readFileSync(
        new URL(
          "../examples/semantic-shot-flagship.shot-submission.json",
          import.meta.url,
        ),
        "utf8",
      ),
  ...
  it("solves a generic partial-limb ground-resting shot into three usable candidates", () => {
    ...
    expect(JSON.stringify(submission.plan)).not.toMatch(
      /joints|quaternion|positionM|rotation/,
    );
    ...
    for (const candidate of result.candidates) {
      ...
      expect(
        candidate.composition.issues.filter(({ severity }) => severity === "error"),
      ).toEqual([]);
      const actor = candidate.scene.entities.find(
        (entity) => entity.id === "actor_generic_1",
      );
      const support = candidate.scene.entities.find(
        (entity) => entity.id === "prop_back_support_1",
      );
      if (!actor || actor.kind !== "actor" || !support) {
        throw new Error("missing solved fixtures");
      }
      ...
      const supportDistance = Math.hypot(...actorToSupport);
      const behindDot = actorForward.reduce(
        (sum, value, index) => sum + value * actorToSupport[index],
        0,
      );
      expect(behindDot).toBeLessThan(0);
      expect(supportDistance).toBeGreaterThanOrEqual(0.18);
      expect(supportDistance).toBeLessThanOrEqual(0.92);
      expect(candidate.metrics.targetCenterNdc[0]).toBeGreaterThan(0.15);
      expect(candidate.worldDiagnostics.pose.contacts).toEqual(
