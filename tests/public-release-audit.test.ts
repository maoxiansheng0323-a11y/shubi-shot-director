...
describe("public release audit", () => {
  it("keeps the v0.10.0 Graybox Craftsman contract generic and publication-safe", async () => {
    const root = fileURLToPath(new URL("../", import.meta.url));
    const releasePath = path.join(root, "docs", "releases", "v0.10.0.md");
    ...
    expect(auditText("docs/releases/v0.10.0.md", releaseSource)).toEqual([]);
  });
